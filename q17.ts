...
let guest_list:string[]=["talat", "shumaila", "rimsha","noureen","Ali","Areeb"]// i make a new original list 
...
while (guest_list.length > 2){
    let removedGuests:string|undefined = guest_list.pop(); // removing the guests more than two
    if(removedGuests!==undefined ){
    console.log(`Sorry ${removedGuests}  we can't invite you for dinner at this time`)};// message guests letting them know that thy are not invited
}
